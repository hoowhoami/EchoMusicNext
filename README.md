...
桌面歌词支持独立悬浮窗显示，适合边工作边听歌时快速查看当前歌词进度。

- **独立窗口**：桌面歌词以独立窗口运行，可与主窗口分离使用。
- **实时同步**：支持歌词行与逐字进度同步，跟随播放状态实时更新。
- **播放控制**：支持在桌面歌词窗口内直接进行播放 / 暂停 / 上一首 / 下一首操作。
- **锁定与穿透**：支持锁定窗口、鼠标穿透与悬浮态切换，减少对日常操作的干扰。
- **样式自定义**：支持主题模式、不透明度、缩放、字体、字号、对齐方式、双行显示等设置。
- **辅助信息**：支持切换翻译歌词 / 音译歌词等副歌词显示模式。
- **窗口调整**：支持拖动位置、调整尺寸，并自动持久化桌面歌词窗口状态。
- **置顶显示**：支持始终置顶与自动显示策略，方便在多任务场景下持续查看。
