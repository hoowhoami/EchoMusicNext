...
export const mapAlbumDetailMeta = (json: unknown): AlbumMeta => {
  ...
  return {
    ...
    singerId: parseIntSafe(
      pickValue(record.author_id, record.singerid, record.singer_id, record.AuthorId, extra.author_id, extra.singer_id, extra.singerid, 0),
    ),
    publishTime: readString(
    ...
    singername: readString(
      pickValue(record.author_name, record.singername, record.singer, extra.singer_name, extra.author_name, ''),
    ) || undefined,
    singerid: parseIntSafe(
      pickValue(record.author_id, record.singerid, record.singer_id, record.AuthorId, extra.author_id, extra.singer_id, extra.singerid, 0),
    ),
